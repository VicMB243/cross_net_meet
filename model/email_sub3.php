...
header("location:../email_sub3.php?oops");
exit();
}
...
$date = date('Y-m-d');
  $email = $key;
  $sql = "SELECT subscription.id,subscription.enddate,organisation.name, organisation.email, subscription.phone,subscription.status,subscription.payment_status
FROM subscription
INNER JOIN organisation ON subscription.organisationid=organisation.id AND subscription.enddate='$date' AND subscription.payment_status='not_paid' WHERE organisation.email = '$email'";
...
header("location:../email_sub3.php?suc");
?>