...
header("location:../email_sub2.php?oops");
exit();
}
...
$mail->setFrom( 'sales@esokoni-markets.com');
  $email = $key;
  $sql = "SELECT subscription.id, organisation.name, organisation.email, subscription.phone,subscription.status,subscription.payment_status
FROM subscription
INNER JOIN organisation ON subscription.organisationid=organisation.id AND subscription.status='pending' AND subscription.payment_status='not_paid'  WHERE organisation.email = '$email'";
...
header("location:../email_sub2.php?suc");
?>